...
SURGE_CONFIG = {
    'history_days': 50,
    'min_rise_percent': 100.0,
    'min_consolidation_days': 14,
    'max_drawdown_percent': 40.0,
    'max_consolidation_new_high_percent': 5.0,
    'breakout_percent': 10.0,
...
def surge_scan(sort_by='breakout', status_filter='all'):
    ...
    for symbol,rs in grouped.items():
        ...
        for i in range(0,max(1,len(rs)-min_cons-2)):
            ...
            for j in range(i+6,high_limit):
                ...
                # Consolidation must remain above the drawdown floor and not keep making highs.
                ...
                current=rs[-1]
                breakout_level=cmax*(1+SURGE_CONFIG['breakout_percent']/100)
                breakout_idx=None
                for k,x in enumerate(cons):
                    if x['price']>=breakout_level: breakout_idx=j+1+k; break
                status='consolidating'; breakout_ts=None; breakout_pct=(current['price']/cmax-1)*100
                if breakout_idx is not None:
                    breakout_ts=rs[breakout_idx]['timestamp']; status='just_broke' if now-breakout_ts<=SURGE_CONFIG['recent_breakout_hours']*3600 else 'broke'
                elif current['price']>=breakout_level:
                    breakout_ts=current['timestamp']; status='just_broke'; breakout_pct=(current['price']/cmax-1)*100
    ...
    return {'timestamp':iso_ts(now),'config':SURGE_CONFIG,'sort':sort_by,'filter':status_filter,'data':results}
